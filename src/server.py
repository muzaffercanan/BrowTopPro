...
def get_logged_users():
    users = []
    utmp_file = "/host/var/run/utmp"  # Path to utmp in the container

    # Structure of utmp: See 'man utmp' for exact layout (depends on platform)
    UTMP_FORMAT = "hi32s4s32s256s"  # Linux-compatible structure
    UTMP_ENTRY_SIZE = struct.calcsize(UTMP_FORMAT)

    try:
        with open(utmp_file, "rb") as file:
            while chunk := file.read(UTMP_ENTRY_SIZE):
                if len(chunk) != UTMP_ENTRY_SIZE:
                    break  # Incomplete entry, stop processing
                ut_type, _, ut_user, ut_line, ut_host, _ = struct.unpack(UTMP_FORMAT, chunk)

                # ut_type == 7 means USER_PROCESS (logged-in user)
                if ut_type == 7:
                    user = ut_user.decode("utf-8", errors="ignore").strip("\x00")
                    tty = ut_line.decode("utf-8", errors="ignore").strip("\x00")
                    host = ut_host.decode("utf-8", errors="ignore").strip("\x00")
                    users.append(f"User: {user}, TTY: {tty}, Host: {host}")

    except FileNotFoundError:
        return ["utmp file not found!"]
    except Exception as e:
        return [f"Error reading utmp: {e}"]
